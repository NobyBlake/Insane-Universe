...
class Particle:
    """Частица с координатами, скоростью и массой."""
...
class World:
    """Мир с размерами, типом границ и методами их обработки."""
    ...
    def apply_boundary(self, particles):
        """Применяет граничные условия ко всем частицам."""
    ...
    def get_minimum_image(self, dx, dy):
        """Возвращает кратчайшее расстояние между точками с учётом периодических границ."""
        if self.boundary == 'torus':
...
class Physics:
    """Расчёт сил и интегрирование уравнений движения."""

    def __init__(self, formula_idx=0, a=1.0, b=0.0, c=1.0, softening=2.0):
    ...
    def compute_force(self, p1, p2, world):
        """Вычисляет силу между двумя частицами по выбранной формуле."""
        dx = p2.x - p1.x
        dy = p2.y - p1.y
        ...
        if self.formula_idx == 0:
            ...
            f = coeff * m1 * m2 * math.exp(-dist / self.c) / dist
        elif self.formula_idx == 5:
            f = coeff * m1 * m2 * math.exp(-dist / self.c) / dist2
        else:
            f = 0.0
    ...
    def compute_accelerations(self, particles, world):
        """Вычисляет ускорения для всех частиц."""
        n = len(particles)
...
class Merger:
    """Объединение частиц при касании."""
    ...
    def get_radius(self, mass):
        """Вычисляет радиус частицы по её массе (2D)."""
        if mass <= 0:
            return 0.1
        return math.sqrt(mass / (math.pi * self.density))

    def find_pairs(self, particles, world):
        """Находит все пары частиц, которые касаются друг друга."""
        pairs = set()
        n = len(particles)
        for i in range(n):
    ...
    def merge(self, particles, world):
        """Объединяет касающиеся частицы в одну."""
...
class RandomGenerator:
    """Генератор случайных частиц по расписанию (на 100 шагов)."""
    ...
    def reset_schedule(self):
        """Генерирует новое расписание для окна из 100 шагов."""
        self.schedule = []
        self.current_step_in_window = 0
    ...
    def get_particles_for_step(self):
        """Возвращает количество частиц, которые должны появиться на текущем шаге."""
        if self.particles_per_100 == 0:
            return 0
...
class Renderer:
    ...
    def get_radius(self, mass):
        """Вычисляет радиус для отрисовки."""
        if mass <= 0:
            return self.min_radius
        return math.sqrt(mass / (math.pi * self.density))

    def mass_to_color(self, mass):
        """Преобразует массу в цвет: от светло-оранжевого (масса 1) до тёмно-оранжевого (масса 100)."""
        if mass <= 1.0:
            return "#FFE4B5"
        if mass >= 100.0:
            return "#8B4513"
    ...
    def draw(self, particles):
        """Рисует все частицы на холсте с центрированием мира."""
...
class Simulation:
    """Основной класс симуляции: содержит все частицы и управляет шагами."""
    ...
    def add_particles(self, count, mass=1.0):
        """Добавляет указанное количество частиц в случайных местах."""
        for _ in range(count):
            self.particles.append(Particle(
                random.uniform(0.0, self.world.width),
                random.uniform(0.0, self.world.height),
                mass=mass
            ))
    ...
    def get_stats(self):
        """Возвращает статистику: количество частиц и номер шага."""
        return {'count': len(self.particles), 'steps': self.step_counter}

    def get_top_masses(self, n=15):
        """Возвращает топ-N масс (по убыванию)."""
        return sorted([p.mass for p in self.particles], reverse=True)[:n]
...
class UniverseApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Insane Universe. Classic")
    ...
    def create_sim_tab(self):
        """Создаёт вкладку управления симуляцией."""
        ...
        tk.Label(c, text="Добавить частицы", fg="white", bg="#1e1e1e",
    ...
    def create_random_tab(self):
        """Создаёт вкладку с настройками случайного добавления частиц."""
        tab = tk.Frame(self.panel, bg="#1e1e1e")
        ...
        tk.Label(c, text="Количество частиц на 100 шагов",
    ...
    def toggle_start_stop(self):
        """Переключает состояние симуляции (старт/стоп)."""
        if self.sim.running:
    ...
    def set_speed(self, val):
        """Обновляет количество шагов за кадр."""
    ...
    def update_stats(self):
        """Обновляет статистику и топ-15 масс."""
        ...
        for i in range(15):
            if i < len(top):
                ...
                if m < 0.001:
                    ...
                    txt = f"{i+1}. {m:.2f}"
                self.top_labels[i].config(text=txt)
            else:
                self.top_labels[i].config(text=f"{i+1}. ---")

    def apply_settings(self):
        """Применяет настройки из вкладки настроек."""
    ...
    def apply_random_settings(self):
        """Применяет настройки случайной генерации."""
